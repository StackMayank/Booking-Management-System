import React from 'react'
import { Button } from "@/components/ui/button"
import { SERVICE_LIST } from '@/config/app.config'
import { Link } from 'react-router-dom'
import { PATHS } from '@/config/path.config'

const Header = () => {
    return (
        <header className='bg-brand py-2'>
            <div className='container flex justify-between items-center'>
                <div id="logo-wrapper">
                    <Link to={PATHS.HOME} aria-label="Go to Booking.com">
                        <img height={25} width={130} src="/assets/booking.com.svg" alt="Logo of Booking.com" />
                    </Link>
                </div>

                <div id="auth"
                    className='flex gap-2 justify-center items-center '>
                    <Button className='bg-white font-semibold text-primary cursor-pointer border-primary rounded-sm hover:bg-white/95'>
                        <Link to={PATHS.SIGN_IN}>Register</Link>
                    </Button>
                    <Button className='bg-white font-semibold text-primary cursor-pointer border-primary rounded-sm hover:bg-white/95'>
                        <Link to={PATHS.SIGN_UP}>Login</Link>
                    </Button>
                </div>
            </div>
            <div className='container flex gap-2 overflow-x-auto scrollbar'>
                {SERVICE_LIST.map((item) => {
                    const ItemIcon = item.icon;
                    return (
                        <Button key={item.id} className={`bg-transparent shadow-none font-medium rounded-full hover:bg-white/50 cursor-pointer flex items-center justify-between gap-2 px-6 h-11 ${item.active ? 'bg-white/10 border border-white/10' : ''} `}>
                            {ItemIcon && <ItemIcon size={18} />}
                            {item.Title}
                        </Button>
                    );
                })}
            </div>
        </header>
    )
}

export default Header